...
const float fastMotionThreshold = 0.6; // Threshold for fast motion
const float slowMotionThreshold = 0.3; // Threshold for slow motion
...
const unsigned long movementCooldown = 5000; // Only process motion every 5 seconds
...
void controlAllLights(bool turnOn, bool weakWhite) {
    for (int i = 1; i <= 52; i++) {
        ...
        for (int j = 0; j < 12; j++) {
            ...
            if (turnOn) {
                if (weakWhite) {
                    sprintf(mqtt_message, "{\"pixelid\": %d, \"R\": 0, \"G\": 0, \"B\": 0, \"W\": 10}", j);
                }
            } else {
